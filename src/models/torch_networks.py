...
class ShiftScaleERPNet(nn.Module):
    ...
    def __init__(self, OUTPUT, config):
        ...
        # Network blocks
        self.mean_net = nn.Linear(self.n_electrodes, self.n_electrodes)
        self.std_net = nn.Linear(self.n_electrodes, self.n_electrodes)
        self.net_1 = nn.Sequential(
            nn.Conv2d(1, 5, kernel_size=(1, 15), stride=1),
            nn.Conv2d(5, 5, kernel_size=(8, 8), stride=1),
            nn.BatchNorm2d(5, momentum=0.1, affine=True))
        self.pool = nn.AvgPool2d(kernel_size=(1, 75), stride=(1, 15))
        self.dropout = nn.Dropout(p=config['DROP_OUT'])
        self.net_2 = nn.Sequential(
            nn.Conv2d(5, OUTPUT, kernel_size=(1, 7), stride=1))
