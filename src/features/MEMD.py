...
def boundary_conditions(indmin, indmax, t, x, z, nbsym):
    ...
    if indmax[-1] < indmin[-1]:
        if x[-1] < x[indmax[-1]]:
            ...
            rmax = np.concatenate(
                (np.array([lx]),
                 np.flipud(indmax[max(end_max - nbsym + 2, 0):])))
        ...
        if x[-1] > x[indmin[-1]]:
            ...
            rmin = np.concatenate(
                (np.array([lx]),
                 np.flipud(indmin[max(end_min - nbsym + 2, 0):])))
            rsym = lx
...
def envelope_mean(m, t, seq, ndir, N, N_dim):  #new
    ...
    for it in range(0, ndir):
        ...
        if mode:
            ...
            env_max = fmax(t)
            amp = amp + np.sqrt(np.sum(np.power(env_max - env_min, 2),
                                       axis=1)) / 2
...
def stop(m, t, sd, sd2, tol, seq, ndir, N, N_dim):
    try:
        ...
        if ((np.mean(sx > sd) > tol or any(sx > sd2))
                and any(nem > 2)) == False:
            stp = 1
        else:
            stp = 0
...
def set_value(*args):
    ...
    if not isinstance(stp_crit, str) or (stp_crit != 'stop'
                                         and stp_crit != 'fix_h'):
        sys.exit(
    ...
    return (q, seq, t, ndir, N_dim, N, sd, sd2, tol, nbit, MAXITERATIONS,
            stp_crit, stp_cnt)
...
def memd(*args):
    ...
    while stop_emd(r, seq, ndir, N_dim) == False:
        ...
        if stop_crit == 'stop':
            ...
            counter = 0
            stop_sift, env_mean, counter = fix(m, t, seq, ndir, stp_cnt,
                                               counter, N, N_dim)
        ...
        if np.max(np.abs(m)) < (1e-10) * (np.max(np.abs(x))):
            if stop_sift == False:
                warnings.warn('emd:warning',
                              'forced stop of EMD : too small amplitude')
            else:
        ...
        while stop_sift == False and nbit < MAXITERATIONS:
            ...
            # computation of mean and stopping criterion
            if stop_crit == 'stop':
                stop_sift, env_mean = stop(m, t, sd, sd2, tol, seq, ndir, N,
                                           N_dim)
            else:
                stop_sift, env_mean, counter = fix(m, t, seq, ndir, stp_cnt,
                                                   counter, N, N_dim)
            ...
            if nbit == (MAXITERATIONS - 1) and nbit > 100:
                warnings.wanr('emd:warning',
                              'forced stop of sifting : too many erations')
