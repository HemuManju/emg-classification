...
def train_test_emd_data(config, leave_out=False):
    ...
    if leave_out:
        features, labels, leave_tags = subject_dependent_emd_data(config)
    else:
        features, labels, leave_tags = subject_independent_emd_data(config)
    ...
    # Add the imf to form the signal
    temp = np.zeros(
        (features.shape[0], config['n_electrodes'], features.shape[2]))
    if config['sum_imf']:
        for i in range(8):
            temp[:, i, :] = np.sum(features[:, i:i + 3, :], axis=1)
        print(temp.shape)
        features = temp
...
def subject_independent_emd_data(config):
    ...
    path = str(Path(__file__).parents[2] / config['clean_emd_data'])
...
def subject_dependent_emd_data(config):
...
def subject_specific_emd_data(subject, config):
